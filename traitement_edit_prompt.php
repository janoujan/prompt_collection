...
if ($id > 0 && $titre && $contenu && $id_type) {
    ...
    $stmt = mysqli_prepare($conn, "UPDATE prompts SET titre = ?, contenu = ?, id_type = ?, id_outil = ?, observation = ?, favori = ? WHERE id = ?");
    // Liaison des paramètres
    mysqli_stmt_bind_param($stmt, "ssiisi", $titre, $contenu, $id_type, $id_outil, $observation, $favori, $id);
    // Exécution
